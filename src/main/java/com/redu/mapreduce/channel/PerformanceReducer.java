...
import com.redu.mapreduce.test.DimensionVO;
import com.redu.mapreduce.test.EmployeePerformanceVO;
...
public class PerformanceReducer extends Reducer<DimensionVO, EmployeePerformanceVO, NullWritable, OrcStruct> {
    ...
    private String channelPartConfigId;

    private static List<PerformanceConfigVO> channelPartConfigValues;
        ...
        for (Path path : paths02) {
            ...
            while (rows.nextBatch(configItemInBatch)) {   // 读1个batch
                for (int i = 0; i < configItemInBatch.size; i++) {
                    ...
                    if ("0".equals(deptId) && channelPartConfigId.equals(id)) {
                        channelPartConfigValues = JSONUtil.toBean(value, new TypeReference<List<PerformanceConfigVO>>() {
                        }, false);
                    }
                }
            }
    ...
    private BigDecimal getPartCommissionWeight(BigDecimal serviceFeeRate, String platform) {
        ...
        for (PerformanceConfigVO configValue : channelPartConfigValues) {
